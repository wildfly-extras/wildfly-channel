...
** A required `groupId` that corresponds to Maven GroupId to pull artifacts. Special syntax `*` can be used to match _any_ groupId. It can only be used in conjunction with `*` artifactId and only a single such stream can be defined.
