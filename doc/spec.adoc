...
** An optional `sha256checksum` that contains the mappings from `[classifier/]extension` to an SHA256 checksum of the artifact files.
...
* the fallback strategy is `latest`, `maven-latest` or `maven-release` but underlying Maven repository contains no metadata for artifacts 
with required `groupId` and `artifactId`. In such a case, no integrity check is done against the resolved artifact.
